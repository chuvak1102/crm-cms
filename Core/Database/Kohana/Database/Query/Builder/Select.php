...
namespace Core\Database\Kohana\Database\Query\Builder;

class Select extends \Core\Database\Database\Query\Builder\Where {
	...
	public function __construct(array $columns = NULL)
	{
		...
		parent::__construct(\Core\Database\Database::SELECT, '');
	}
	...
	{
		$this->_join[] = $this->_last_join = new \Core\Database\Database\Query\Builder\Join($table, $type);
	...
	public function union($select, $all = TRUE)
	{
		if (is_string($select))
		{
			$select = \Core\Database\DB::select()->from($select);
		}
		if ( ! $select instanceof \Core\Database\Database\Query\Builder\Select)
	...
	public function compile($db = NULL)
	{
		if ( ! is_object($db))
		{
			...
			$db = \Core\Database\Database::instance($db);
		}
